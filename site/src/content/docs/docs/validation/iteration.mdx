...
## Corpus-wide confirmation (v2 sweep)

After the kammer single-paper iteration validated the prompt change, a full
10-paper sweep (`evaluate --all --review-mode external` with the iterated
prompt) confirmed the change is a net win across the corpus.

| Metric | v1 baseline (4-bias reviewer) | **v2 (7-bias reviewer)** | Δ |
|:-------|:------------------------------:|:------------------------:|:--:|
...
The pattern is clear: targeted iteration on a known failure mode generalizes
across the corpus, with predictable trade-offs (a paper structurally similar
to the failure mode helps; a paper structurally different may regress slightly).
This is the iteration discipline working as designed — every change validated
empirically before deployment, deltas inscribed for the worklog.

Cost of the v2 confirmation sweep: ~$100. The v1 → v2 transition is recommended
as the new default; rozak and kolb regressions are documented for future targeted
iteration via `--prompt-variant`.
...
- [The 10-paper sweep results](/elife-claim-trees/docs/validation/results/) — the baseline you'd compare iterations against
- [Adding a prompt variant](/elife-claim-trees/docs/contributing/prompt-variant/) — for non-default-target iterations
- [Known limitations](/elife-claim-trees/docs/validation/limitations/) — what's worth iterating vs accepting
