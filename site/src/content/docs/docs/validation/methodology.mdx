...
For matched claims (recovery numerator), did the CLI assign the same panel as the curator? Multi-panel reference claims (`panel: fig4, fig5`) often score panel-mismatch when the CLI emits a single-panel claim with the same content; this is the [documented panel limitation](/docs/validation/limitations/).
...
The kammer iteration is the canonical worked example — see [iteration discipline](/docs/validation/iteration/) for the case study.
...
- [The 10-paper sweep results](/docs/validation/results/) — the empirical case for the system
- [Known limitations](/docs/validation/limitations/) — what the metrics tell us is hard
