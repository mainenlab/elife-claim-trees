...
- **kammer's failure** is reviewer-prompt under-coverage of the `control` role. A targeted prompt iteration moved kammer from 57% to 71% on a single change (still below threshold, but +14 points); see [iteration discipline](/elife-claim-trees/docs/validation/iteration/) for the worked example.
...
- [Iteration discipline](/elife-claim-trees/docs/validation/iteration/) — the kammer worked example for prompt iteration
- [Cost and performance](/elife-claim-trees/docs/cli/cost/) — operational tradeoffs
- [Adding a prompt variant](/elife-claim-trees/docs/contributing/prompt-variant/) — for domain or paper-type customization
