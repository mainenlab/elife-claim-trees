...
Targeted iteration on a single paper before committing to the full sweep is the right pattern. The kammer iteration ([iteration discipline](/elife-claim-trees/docs/validation/iteration/)) is the worked example: ~$10 to validate a prompt change on the target paper, ~$100 to confirm the change is a net win across the corpus.
...
- [Running validation](/elife-claim-trees/docs/contributing/validation/) — the workflow for evaluate-driven measurement
- [Iteration discipline](/elife-claim-trees/docs/validation/iteration/) — kammer worked example
- [The prompts](/elife-claim-trees/docs/reference/prompts/) — full reference for what's in each default prompt
