...
**Verify-refs.** A CLI subcommand that resolves DOIs for `role: literature-context` claims via CrossRef. Anti-hallucination check on existing DOIs; resolution from Author (Year) hints when DOIs are missing. See [the five subcommands](/elife-claim-trees/docs/cli/subcommands/).
