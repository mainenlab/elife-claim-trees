...
The `run` subcommand chains `extract` + `write --review-mode auto-approve` + `verify-refs`. Note: `run` defaults to **auto-approve**, not external. For bulk operation with the reviewer pass, run `extract` and `write --review-mode external` separately, or use `evaluate` (see [batch operation](/elife-claim-trees/docs/cli/batch/)).
...
- Tune the model selection — see [cost and performance](/elife-claim-trees/docs/cli/cost/)
- Hand the claim files to a curator for the high-value review — see [the review gate](/elife-claim-trees/docs/architecture/review-gate/)
