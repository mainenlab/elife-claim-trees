...
For the architectural rationale (why a review gate exists at all), see [the review gate](/elife-claim-trees/docs/architecture/review-gate/).
...
Output: ~78% role agreement on average (matches the [10-paper sweep](/elife-claim-trees/docs/validation/results/)). For peer-review augmentation, this quality is sufficient — reviewers see structured claims, citations are anti-hallucination-checked, the schema is consistent.
...
- [Batch operation](/elife-claim-trees/docs/cli/batch/) — running on many papers efficiently
- [Cost and performance](/elife-claim-trees/docs/cli/cost/) — per-step / per-model breakdown
- [The external reviewer](/elife-claim-trees/docs/architecture/external-reviewer/) — full design of the Opus pass
