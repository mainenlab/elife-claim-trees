...
You can run this exact sequence yourself once you have the CLI [installed and configured](/elife-claim-trees/docs/cli/install/). Total wall time: ~15 minutes. Total cost: ~$7 in mixed Sonnet 4.6 and Opus 4.6 calls via Vertex AI.
...
See [the three-agent partition](/elife-claim-trees/docs/architecture/three-agent-partition/) for why the partition is structured this way.
...
Four review modes are available. For this demo we pick `external` — an Opus pass that recovers the deductive structure (hypotheses, predictions) the prose-level extraction systematically misses. See [the review gate](/elife-claim-trees/docs/architecture/review-gate/) for when to use which mode.
...
- See the [pipeline diagram](/elife-claim-trees/docs/architecture/pipeline/) for the architectural overview
- Read the [external reviewer page](/elife-claim-trees/docs/architecture/external-reviewer/) — that's the system's most novel decision
- Try it yourself via the [install guide](/elife-claim-trees/docs/cli/install/)
- Look at the [validation results](/elife-claim-trees/docs/validation/results/) for how the same pipeline performs across the 10-paper public eLife corpus
