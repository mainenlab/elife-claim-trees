...
Runs an Opus pass that substitutes for the analyst at the review gate. The reviewer reads the paper plus the draft and produces a revised draft addressing the seven systematic biases prose-level extraction exhibits (see [the external reviewer](/elife-claim-trees/docs/architecture/external-reviewer/) for the full design).
...
- [The external reviewer (Step 4.5)](/elife-claim-trees/docs/architecture/external-reviewer/) — full design of what `external` mode does
- [Review modes — when to use each](/elife-claim-trees/docs/cli/review-modes/) — operational decision tree from the CLI's perspective
- [Validation results](/elife-claim-trees/docs/validation/results/) — empirical performance of each mode across the 10-paper public corpus
